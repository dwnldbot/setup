...
echo "$(date): $(pfw) " >> $HOME/focustest.txt
...
echo "$(date): arg $1: cur $CUR: $wid as focus.sh prev, $(lsw|grep -v $CUR|sed '1 p;d') as next, $(lsw)" >> $HOME/focusprevtest.txt
